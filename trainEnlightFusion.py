    ...
    for epoch in range(args.epochs):
        ...
        for it, (image_vis, image_ir, name, bi) in enumerate(fusion_train_loader):
            ...
            if num > 0:
                ...
                lossTotal = loss
            print('Train enlight - Loss: {}, Num: {}'.format(lossTotal.item(), num))
            lossTotal.backward()
...
def train_fusion(i):
    ...
    for epo in range(0, epoch):
        for it, (image_vis, image_ir, name, bi) in enumerate(train_loader):
            ...
            loss_total.backward()
            optimizer.step()
            printLossInfo(benCiStartTime, bi_loss, epo, epoch, glob_StartTime, it, loss_grad, loss_in, loss_total,
                          train_loader)
...
def printLossInfo(benCiStartTime, bi_loss, epo, epoch, glob_StartTime, it, loss_grad, loss_in, loss_total,
                  train_loader):
    endTime = time.time()
...
def test_Fuse():
    ...
    with torch.no_grad():
        for it, (images_vis, images_ir, name) in enumerate(test_loader):
            ...
            logits, binary_out = fusionmodel(images_vis_ycrcb,
                                             images_ir)
            print('gen the fuisonImageY  Sucessfully!')
            fusion_ycrcb = torch.cat(
                (logits, images_vis_ycrcb[:, 1:2, :,
                         :], images_vis_ycrcb[:, 2:, :, :]),
                dim=1,
...
if __name__ == "__main__":
    ...
    for i in range(0, alternate):
        train_Elight(i)
        print("|{0} Train Enlight Model Sucessfully~!".format(i + 1))
        run_Elight()
        print("|{0} Enlight Image Sucessfully~!".format(i + 1))
        train_Fuse(i)
    print("training Done!")
